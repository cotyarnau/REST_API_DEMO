...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (StringUtils.hasText(token) && jwtGenerador.validarToken(token)) {
            // Asignamos el nombre de usuario contenido en el objeto "token" y lo pasamos a
            // nuestra variable "username"
            String username = jwtGenerador.obtenerUsernameDeJwt(token);
            // Luego creamos el objeto userDetails el cual contendrá todos los detalles de
            // nuestro username, ósea nombre, pw y roles según el método loadUserByUsername
            UserDetails userDetails = customUsersDetailsService.loadUserByUsername(username);
            // Cargamos una lista de String con los roles alojados en BD
            List<String> userRoles = userDetails.getAuthorities().stream().map(GrantedAuthority::getAuthority).toList();
            // Comprobamos que el usuario autenticado posee alguno de los siguientes roles
            // alojados en BD
            if (userRoles.contains("USER") || userRoles.contains("ADMIN")) {
                /*
                 * Creamos el objeto UsernamePasswordAuthenticationToken el cual contendrá los
                 * detalles de autenticación del usuario
                 */
                UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(
                        userDetails,
                        null, userDetails.getAuthorities());
                // Aca establecimos información adicional de la autenticación, como por ejemplo
                // la dirección ip del usuario, o el agente de usuario para hacer la solicitud
                // etc.
                authenticationToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
                // Establecemos el objeto anterior (autenticación del usuario) en el contexto de
                // seguridad
                SecurityContextHolder.getContext().setAuthentication(authenticationToken);
            }
        }
        // Permite que la solicitud continue hacia el siguiente filtro en la cadena de
        // filtro.
        filterChain.doFilter(request, response);

        // Este filter después va a ser utilizado para crear un bean en la
